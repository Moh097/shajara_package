...
# ---------- Prime env ----------
export_secrets_to_env()  # makes SUPABASE_* etc available to utils.supabase_client
...
# We no longer use local SQLite; collectors + app both talk to Supabase via modules.data_access.
# If you want to keep SHAJARA_DB_PATH for backwards compat, you can leave it in env, but it's unused now.
...
# Supabase-backed total rows
def _db_total_rows_supabase() -> int:
    try:
        return db_total_rows()
    except Exception:
        return 0


# Supabase-backed latest rows for channels (used in collection preview)
def _fetch_latest_for_channels_supabase(channels: List[str], limit_rows: int = 200) -> pd.DataFrame:
    try:
        return fetch_latest_for_channels(channels, limit_rows)
    except Exception:
        return pd.DataFrame()
...
def _run_collector(module_path: str, label: str):
    env = os.environ.copy()
    # SHAJARA_DB_PATH no longer used for DB access; collectors should be using Supabase too.
    # Keeping it here for backwards-compat if your collectors still read it.
    env.setdefault("SHAJARA_DB_PATH", "shajara.db")
...
if "collect_result" in st.session_state:
    ...
    with busy_popup("Building collection preview…"):
        preview_df = _fetch_latest_for_channels_supabase(
            cr.get("channels_used", []),
            limit_rows=min(200, max(50, cr.get("upserted_total", 200))),
        )
    if not preview_df.empty:
        st.subheader("Latest rows from collected channels (Supabase)")
        st.dataframe(preview_df, use_container_width=True, height=300)
    else:
        st.info("No preview rows (maybe channels list was empty, collector wrote nothing, or Supabase is empty).")
...
total_rows = _db_total_rows_supabase()
st.info(
    f"Showing **{len(df)}** rows (after filters) out of **~{total_rows}** total in Supabase. "
...
st.success(f"Loaded {len(df)} rows from Supabase posts table.")
...
c4.metric(
    "Median (GPT tokens)",
    int(stat_df["gpt_tokens"].dropna().median())
    if not stat_df.empty and stat_df["gpt_tokens"].notna().any()
    else 0,
)

with st.expander("Show table", expanded=True):
    st.dataframe(
        df[["datetime_utc", "source_name", "author", "text", "likes", "shares", "comments"]],
        use_container_width=True,
        height=400,
    )
...
if run_analysis:
    ...
    with busy_popup("Analyzing posts & building visuals…"):
        ...
        if not topic_map:
            topic_map = {k: k for k in uniq_topics}
        ent_map = gpt_semantic_merge_terms(uniq_entities) if uniq_entities else {}
        if not ent_map:
            ent_map = {k: k for k in uniq_entities}
        ...
        if wc_buf is None:
            ...
            st.download_button(
                ...
                data=img,
                file_name="entities_wordcloud.png",
                mime="image/png",
            )
else:
